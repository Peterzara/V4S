...
-- bd=[q(x1,x2,...), r(x1,x2,...)] base = base head = h(x1,x2)
...
-- Helper functions
data NState = N { nxv :: Int} deriving (Show) -- x1,... x_nxv
type NM = State NState -- State NState a
...
normalizeVar e = do
        fresh_var <- nxVar
        return (fresh_var, Eq fresh_var e)
-----------------------------------------------
normalizeQuery :: Query -> NM (Query, [Base])
-----------------------------------------------
normalizeQuery p = do
        let normalizeQueryHelper [] = do
                return []
            normalizeQueryHelper ((Var x):vs) = 
              do
                varsNewRest <- (normalizeQueryHelper vs) 
                return $ [((Var x),Tr)] ++ varsNewRest

            normalizeQueryHelper (e:vs) = 
              do
                varsNewRest <- (normalizeQueryHelper vs)
                normalizedVar <- normalizeVar e
                return $ [normalizedVar] ++ varsNewRest
            

        var2Base <- normalizeQueryHelper (vars p)
        let tmp = unzip var2Base
            new_query = Query {name=(name p), vars=fst tmp}
            new_base = [ x | x <- snd tmp, x /= Tr]
        return (new_query, new_base)
...
normalizeHorn h = do
        ...
        --        ,      annot :: a
        --        }
        tuple_hd <- normalizeQuery (hd h)
        
        let normalizeHornHelper [] = do
                return []

            normalizeHornHelper (x:vs) = do
                varsNewRest <- normalizeHornHelper vs
                normalizedQuery <- normalizeQuery x
                return $ [normalizedQuery] ++ varsNewRest

        query2Base <- normalizeHornHelper (bd h)
        let tmp = unzip query2Base -- ([query], [[base],[base]])
            new_bd = fst tmp
            new_base = And $ [base h] ++ (snd tuple_hd) ++ concat (snd tmp)
            new_hd = fst tuple_hd
        return Horn {hd = new_hd, bd = new_bd, base=new_base, annot=annot h}
...
-----------------------------------------------
normalizeBoundM b = do
        -- data Bound  = Bound { 
        --               queries  :: [Query] 
        --        ,      bbase    :: Base
        --        ,      bound    :: Base
        --       }
        let normalizeBoundHelper [] = do
                return []
            normalizeBoundHelper (x:vs) = do
                varsNewRest <- normalizeBoundHelper vs
                normalizedQuery <- normalizeQuery x
                return $ [normalizedQuery] ++ varsNewRest

        query2Base <- normalizeBoundHelper (queries b)
        let tmp = unzip query2Base
            new_quereis = fst tmp
            new_base = And $ [bbase b] ++ concat (snd tmp)
        return Bound {queries = new_quereis, bbase=new_base, bound=bound b}
